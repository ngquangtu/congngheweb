<?php
    ...
    if($_POST){
        //$abc = @mysqli_real_escape_string($conn,$_POST['abc']);
        $id = $_POST['addID'];
        $ten = $_POST['addName'];
        $sdt = $_POST['addPhone'];
        ...
        $sqlCheck = "SELECT * FROM giangvien WHERE ID = '$id'";
        $check = mysqli_query($conn, $sqlCheck);
        $count = mysqli_num_rows($check);
        if($count ==  0){  
            $sql = "INSERT INTO giangvien(ID,Hoten,SDT,Diachi,email,chucvu,bomon,ID_tk) VALUES ($id,'$ten','$sdt','$diachi','$Email','$role','$nganh',$id)";
            $sql1 = "INSERT INTO historydel(Hoten,SDT,Diachi,email,chucvu,bomon,thoigian,tacvu,nguoithuchien) VALUES ('$ten','$sdt','$diachi','$Email','$role','$nganh','$date','Thêm','$name')";
